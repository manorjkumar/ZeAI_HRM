...
class _EventBannerSliderState extends State<EventBannerSlider> {
  ...
  try {
    ...
    final responseProfiles = await http.get(
      Uri.parse('https://zeai-hrm-1.onrender.com/profile'),
    );
    ...
    final responseHolidays = await http.get(
      Uri.parse(
        'https://zeai-hrm-1.onrender.com/notifications/holiday/employee/ALL?month=$currentMonthName',
      ),
    );
    ...
    final responseCompanyEvents = await http.get(
      Uri.parse('https://zeai-hrm-1.onrender.com/company-events'),
    );
