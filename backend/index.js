...
app.use(cors({
  origin: ["https://hrmzeaisoftverbeta.netlify.app"],
...
// -------------------- NEW SIGNALING SERVER (Cross-Platform) -------------------- //
...
io.on("connection", (socket) => {
  ...
  socket.on("register", (userId) => {
    ...
    console.log(`✅ ${userId} registered (${socket.id})`);
  });
  ...
  socket.on("candidate", (data) => {
    const target = users[data.to];
    if (target) io.to(target).emit("candidate", data);
  });

  // End call
  socket.on("call-ended", (data) => {
    const target = users[data.to];
    if (target) io.to(target).emit("call-ended", data);
  });
...
// ---------------- ROOT ROUTE ---------------- //
app.get('/', (req, res) => {
  res.send('✅ HRM Backend & Signaling Server Running Successfully!');
});
